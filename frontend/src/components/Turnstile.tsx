...
const SITE_KEY = "0x4AAAAAAFC1qutvBzDcdpGt";
...
function loadTurnstile(): Promise<TurnstileApi> {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (existing) {
      existing.addEventListener("load", () => window.turnstile && resolve(window.turnstile), { once: true });
...
export function Turnstile({ onToken }: { onToken: (token: string | null) => void }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("scribble:turnstile-reset", reset);
    return () => {
      active = false;
      window.removeEventListener("scribble:turnstile-reset", reset);
