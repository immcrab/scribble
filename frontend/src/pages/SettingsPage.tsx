...
import { PuterNoticeModal } from "../components/PuterNoticeModal";
...
export function SettingsPage({ tab: tabParam, onExit }: { tab: string; onExit: () => void }) {
  const { settings, updateSettings } = useChatStore();
  const user = useAuthStore((s) => s.user);
  const signInWithGoogle = useAuthStore((s) => s.signInWithGoogle);
  const [tab, setTabState] = useState<Tab>(isTab(tabParam) ? tabParam : "general");
  ...
  const setTab = (next: Tab) => {
    setTabState(next);
    window.history.replaceState(null, "", settingsPath(next));
  };
  ...
  return (
    <>
      <div className="flex h-dvh w-full flex-col overflow-hidden bg-base-950">
        <div className="flex items-center gap-3 border-b border-base-700/60 px-4 py-3 sm:px-6">
          <button onClick={onExit} className="rounded-lg p-1.5 text-slate-400 hover:bg-base-700 hover:text-white" title="Back to chat">
            <ArrowLeft size={18} />
          </button>
          <h1 className="text-lg font-semibold text-white">Settings</h1>
        </div>

        <div className="flex gap-1 overflow-x-auto border-b border-base-700/60 px-4 sm:px-6">
          {TABS.map(({ id, label, icon: Icon }) => (
            <button
              key={id}
              onClick={() => setTab(id)}
              className={`flex shrink-0 items-center gap-1.5 border-b-2 px-3 py-2.5 text-sm font-medium transition-colors ${
                tab === id ? "border-accent-500 text-white" : "border-transparent text-slate-500 hover:text-slate-300"
              }`}
            >
              <Icon size={14} />
              {label}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
          <div className="mx-auto w-full max-w-lg">
            ...
            <div className="mt-8 flex gap-3 border-t border-base-700/60 pt-4 text-xs text-slate-500">
